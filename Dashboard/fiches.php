...
<?php 
	$outuser = Grid4PHP::getGrid('db_contacts', $usrActif); 
...
<div id="page-content" class="">
		...
		<script>
			var opts = {
				...
				'ondblClickRow': function (id) {
					...
					if (jQuery(this).attr('id') == 'list_fiches') {
			...
			<div class="col-md-12 col-lg-12">
				<div class="block-options pull-right" id="idmob"> 
					...
					<div class="btn-group btn-group-sm">
						...
						<!-- <a href="#" style="margin: 5px;" class="btn btn-xs btn-primary btcleanroom" <?php echo $arrAccess['edit_clean_room'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-bath"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Nettoyage chambres' : 'Cleaning rooms') ?></a> -->
					
						<a href="#" style="margin: 5px;" class="btn btn-xs btn-warning btemaillibre" <?php echo $arrAccess['email_libre'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-enveloppe-o"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Email Libre' : 'Free email') ?></a>
						
						<!-- <a href="#" style="margin: 5px;" class="btn btn-xs btn-info btnewfiche" <?php echo $arrAccess['create_client'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-plus-circle"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Nouveau Client' : 'New customer') ?></a> -->
						
						<a href="#" style="margin: 5px;" class="btn btn-xs btn-danger dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-check-square-o"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Actions sur sélection(s)' : 'Actions on slection(s)') ?> <span class="caret"></span></a>
						
						<ul class="dropdown-menu dropdown-menu-right">
							<li>
								<a href="#" id="btroomlisttab" <?php echo $arrAccess['print_list_room'] != '1' ? 'style="display:none"' : "" ?>	><i class="fa fa-bed"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'liste des chambres (Tableau)' : 'Room list (Table)') ?></a>
							</li>
							<li>
								<a href="#" id="btroomlistpage" <?php echo $arrAccess['print_list_room'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-bed"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'liste des chambres (Page)' : 'Room list (Page)') ?></a>
							</li>
							<li>
								<a href="#" id="btdelct" <?php echo $arrAccess['del_client'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-trash"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Supprimer' : 'Delete') ?></a>
							</li>
							<li>
								<a href="#" id="btmailgroup" <?php echo $arrAccess['send_mail_client'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-envelope"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Mail multiple' : 'Multiple email') ?></a>
							</li>
							<li>
								<a href="#" id="btsmsgroup" <?php echo $arrAccess['send_sms_client'] != '1' ? 'style="display:none"' : "" ?>><i class="fa fa-send"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'SMS multiple' : 'Multiple SMS') ?></a>
							</li>
							<?php if ($arrAccess['change_statut_client'] == '1') { ?>
								<li>
									<a href="#" id="btstatussec"><i class="fa fa-info"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Changer l\'état du dossier' : 'Change status of folder') ?></a>
								</li>
							<?php } ?>
						</ul>
					</div>

					<a href="#" id="initgrid" style="margin-right:13px; height:30px; text-align:center; padding-top:5px; color:#000" class="btn btn-xs btn-warning"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Raffraichissement des données' : 'Refresh data') ?></a>
				</div>
	...
	<div id="modal-changestatussec" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				
				<div class="modal-header text-center">
					<h2 class="modal-title"><i class="fa fa-exchange"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Changer l\'état du dossier' : 'Change folder status') ?></h2>
				</div>
				
				<div class="modal-body">
					<form action="appajax.php" method="post" enctype="multipart/form-data" class="form-horizontal" id="formulairechangestatussec" onsubmit="return false;">
						<fieldset>                        
							<div class="form-group">
								<label class="col-md-3 control-label" for="id_status_sec"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Statut' : 'Status') ?></label>
								<div class="col-md-9">
									<select id="id_status_sec" name="id_status_sec" class="form-control">
										<option value="0"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Sélectionner le statut' : 'Choice of status') ?></option>
										<?php
											$sts = Setting::getAllStSec();
											if ($sts) {
												foreach($sts as $st) {
													echo '<option value="'.$st['id_status_sec'].'">'.$st['name_status_sec'].'</option>';
												}
											}
										?>
									</select>
							...
							<div class="form-group form-actions">
								<div class="col-xs-12 text-right">
									...
									<input type="hidden" name="action" id="action" value="change-status-rows" />
									<a href="#" class="btn btn-sm btn-default" data-dismiss="modal"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Quitter' : 'Close') ?></a>
									<button type="submit" href="#" class="btn btn-sm btn-primary" id="btchangestatus"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Valider' : 'Confirm') ?></button>
								</div>
							</div>
						</fieldset>
					</form>
	...
	<div id="modal-save-view" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				
				<div class="modal-header text-center">
					<h2 class="modal-title"><i class="fa fa-floppy-o"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Sauvegarder la nouvelle vue' : 'Save view') ?></h2>
				</div>
				
				<div class="modal-body">
					<form action="appajax.php" method="post" class="form-horizontal" id="formulairesavestate" onsubmit="return false;">
						<fieldset>
							<div class="form-group">
								<label class="col-md-3 control-label" for="namestate"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Nom de la vue' : 'View name') ?></label>
								<div class="col-md-9">
							...
							<div class="form-group form-actions">
								<div class="col-xs-12 text-center">
									<a href="#" class="btn btn-sm btn-default" data-dismiss="modal"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Annuler' : 'Cancel') ?></a>
									<button type="submit" href="#" class="btn btn-sm btn-primary" id="btsavestate"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Valider' : 'Confirmt') ?></button>
								</div>
							</div>
						</fieldset>
					</form>
	...
	<div id="modal-comment" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header text-center">
					<h2 class="modal-title"><i class="fa fa-commenting-o"></i> <?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Ajoutez un commentaire' : 'Add comment') ?></h2>
				</div>
				
				<div class="modal-body">	
					<form action="appajax.php" method="post" class="form-horizontal " id="formulairecommentgrid"  onsubmit="return false;">
						<div class="form-group">							
							<textarea placeholder="<?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Nouveau commentaire' : 'New comment') ?>" class="form-control" name="comment" id="comment-grid" style="height: 190px;"></textarea>								
						</div>
						<div class="form-group form-actions">
							<div class="col-xs-12 text-right">
								<input type="hidden" name="action" id="action" value="add-comment-grid" />
								<input type="hidden" name="id_contact_comment" id="id_contact_comment" value="" />
								...
								<button type="submit" class="btn btn-sm btn-primary" id="btaddcomment"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Valider' : 'Confirm') ?></button>
							</div>
						</div>
					</form>
				</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form onsubmit="return false;" class="form-horizontal" id="formulairerecall" method="post" action="appajax.php">
						...
						<div class="form-group form-actions">
							<div class="col-md-12 text-center">
								<input type="hidden" name="id_fiche" id="id_contact_rappel" value="" />
								<input type="hidden" name="action" id="rappel" value="add-rappel" />
								<button href="#" class="btn btn-sm btn-default" data-dismiss="modal"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Quitter' : 'Close') ?></button>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body" style="height: 300px;">
					...
					<div class="form-group form-actions">
						<div class="col-md-12 pull-left" style="display:flex;margin-bottom:10px">
						...
						<div class="col-md-12 pull-left">
							<button href="#" class="pull-right btn btn-sm btn-danger" data-dismiss="modal"><?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Quitter' : 'Close') ?></button>
						</div>
					</div>
				</div>
			</div>
...
<script>
	...
	$(document).ready(function() {
		...
		if(window.matchMedia("(max-width: 600px)").matches){
			...
			if(window.matchMedia("(min-width: 900px) and (max-width: 911px) and (orientation: portrait)")){
				$('#idmob').removeClass('pull-right');
			}else{
		...
		$('#btsendinf').click(function(){
			...
			$.post('appajax.php', {
				...
				if (resp.responseAjax == "SUCCESS") {
					...
					$('#modal-new-fiche').modal('hide')
					$('#list_fiches').trigger("reloadGrid",[{current:true}]);
		...
		$(document).on('click','.btroom',function(){
			...
			// var selrows = $('#list_fiches').jqGrid('getGridParam','selarrrow')
		...
		$('#btmailsending').click(function() {
			...
			jQuery('#formulairemsgdoc').ajaxSubmit({
				dataType:'json',
				data:{action:'send-mail-grid', rws:$('#ficheid').val(), idmailtype: $('#id_mailtype').val(), subject:$('#subjectmail').val(), msg:CKEDITOR.instances['msgcontent'].getData()},
				success : function (resp) {	
					HoldOn.close();
					if (resp.responseAjax == 'SUCCESS') {					
						$.bootstrapGrowl('<h4>Confirmation!</h4> <p>Email envoyé</p>', {
		...
		$('#btsavestate').click(function() {
			//var savedState = JSON.stringify($("#list_fiches").getGridParam()); // jQuery.jStorage.get('localSstorage-lct');
			var savedState = jQuery.jStorage.get('localSstorage-lct');
		...
		$(document).on('click', '.mdcol', function() {
			...
			if (savedState[key] != null) {
				jQuery.jStorage.set('localSstorage-lct', savedState[key].val);
		...
		$(document).on('click', '.mdcol ', function() {
			if (confirm('<?php echo ($usrActif->lang == 'FR' || $usrActif->lang == '' ? 'Confirmez la suppression de la vue ?' : 'Confirm deletion of view ?') ?>')) {
				...
				//$('#list_fiches').gridState().remove('localSstorage-lct');
		...
		function loadsavedState() {
			var savedState = JSON.parse(localStorage.getItem('storageviews'));
			if (savedState != null) {
				var cursavedState = JSON.stringify(jQuery.jStorage.get('localSstorage-lct'));
	...
	function gridcts_onload(ids) {	
		if(ids.rows) 
			jQuery.each(ids.rows,function(i) {
				...
				if (this.status_color != '' && this.status_color != undefined)	
					jQuery('#list_fiches tr.jqgrow:eq('+i+')').css('background-image','inherit').css({'background-color':this.status_color});
			});
		...
		$('.ui-pg-selbox').val(jQuery('#list_fiches').getGridParam('rowNum'));
		jQuery('#list_fiches').jqGrid('resetSelection');
	}
